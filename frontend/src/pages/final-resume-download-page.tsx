...
export function FinalResumeDownloadPage() {
  ...
  const isReady = !!state.generation.completedAt && !!pdfUrl;
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8 lg:py-12">
        ...
        >
          ...
          <div className="space-y-8">
            ...
            >
              <div className="rounded-[2.2rem] border border-white/10 bg-[linear-gradient(180deg,#15191d,#0d1013)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.28)] sm:p-8 flex flex-col">
                ...
                <div className="flex-1 min-h-[45rem] relative rounded-[1.8rem] border border-white/10 bg-white/[0.02] overflow-hidden flex flex-col items-center justify-center">
                  {isGenerating ? (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#07090c]/80 backdrop-blur-sm p-6 text-center">
                      <LoaderCircle className="h-10 w-10 animate-spin text-[#dfc497] mb-6" />
                      <p className="text-lg font-medium text-white mb-6">Generating your resume...</p>
                      <div className="space-y-3 w-full max-w-md text-left">
                        {generationStages.map((stage) => (
                          <div key={stage.title} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 flex items-center justify-between gap-3">
                            ...
                            <span className={`rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.2em] ${
                              stage.status === "complete" ? "bg-emerald-400/12 text-emerald-200" :
                              stage.status === "active" ? "bg-[#dfc497]/12 text-[#f2dfbe]" :
                  ...
                  {isReady ? (
                    <iframe
                      src={`${pdfUrl}#view=Fit&navpanes=0&toolbar=1`}
                      className="w-full h-full bg-white absolute inset-0 rounded-[1.8rem]"
                    ...
                    <div className="max-w-lg text-center p-6">
                      ...
                      <p className="mt-5 text-lg font-medium text-white">Generation has not started yet.</p>
                      <p className="mt-3 text-sm leading-7 text-[#c4beb2]">
                        Ensure your details are filled out in the Intake tab, then click the Generate button.
                      </p>
                    </div>
                  ) : null}
                </div>
                ...
                <div className="rounded-[2.2rem] border border-white/10 bg-[linear-gradient(180deg,#15191d,#0d1013)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
                  ...
                  <div className="mt-6 space-y-3">
                    ...
                    <motion.a
                      ...
                      className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 text-left text-[#d8d2c6] transition-colors hover:bg-white/[0.06]"
                    >
                ...
                <div className="rounded-[2.2rem] border border-white/10 bg-[linear-gradient(180deg,#15191d,#0d1013)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
                  ...
                  <div className="mt-5 space-y-3">
                    <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-4 text-sm leading-7 text-[#d8d2c6]">
                      Current file: <span className="text-white break-all">{downloadFileName}</span>
                    </div>
                    <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-4 text-sm leading-7 text-[#d8d2c6]">
                      Output status:{" "}
